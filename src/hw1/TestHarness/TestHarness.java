...
public class TestHarness {	
	...
	public boolean doWork(String[] args,Socket sock, HashMap<String,Object> att) throws Exception {
		...
		for (int i = 1; i < args.length - 1; i += 2) {
			...
			response.addHeader("Protocol", requestParams.get("requestVersion").toString());
			//
		...
		//	System.out.println("requestURL "+request.getAttribute("requestURL"));
			...
			if (args[i].compareTo("GET") == 0 || args[i].compareTo("POST") == 0) {
				...
				writeHeader(sock.getOutputStream(), response);
				
				writeBody(sock.getOutputStream(),response);
	...
	private void writeBody(OutputStream out, MyResponse response) throws IOException {
	...
	private void writeHeader(OutputStream out, MyResponse response)
			throws IOException {
